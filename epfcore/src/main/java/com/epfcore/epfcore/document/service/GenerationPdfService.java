...
public class GenerationPdfService {
    ...
    public byte[] generateFromRequest(Integer studentId) {
        ...
        if (existingInfos.isPresent()) {
            String fileData = existingInfos.get().getFileUrl();
            if (fileData != null) {
    ...
    private String buildHtml(Student student) {
        ...
        return """
                ...
                """.formatted(bourseFormatee, campus, academicYear, nom, prenom, email, nationality, birthDateFormatee,
                numeroFormate, address, numero, major, lastDegree, EnrollmentDateFormatee);
    }
